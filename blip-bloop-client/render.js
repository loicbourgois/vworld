const render = () => {
  ...
  for (let puuid in chunk.particles) {
    const particle = chunk.particles[puuid]
    const x = particle.x / chunk.width;
    const y = particle.y / chunk.height;
    const d = particle.d / chunk.width;
    draw_body(canvas_1, x, y, d, zoom, center_x, center_y)
    draw_body(canvas_2, x, y, d, 1.0, 0.5, 0.5)
  }
  for (let luuid in chunk.links) {
    const link = chunk.links[luuid]
    const puuid_a = link[0]
    const puuid_b = link[1]
    const p_a = chunk.particles[puuid_a]
    const p_b = chunk.particles[puuid_b]
    const x_a = p_a.x / chunk.width;
    const y_a = p_a.y / chunk.height;
    const x_b = p_b.x / chunk.width;
    const y_b = p_b.y / chunk.height;
    draw_link(x_a, y_a, x_b, y_b, zoom)
  }
  return;
